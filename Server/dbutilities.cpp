...
int db::removeFriend(const char *path,const char* username, int id1,int socket)
{
    sqlite3 *db;
    if (sqlite3_open(path, &db))
    {
    ...
    sprintf(sql, "DELETE FROM friends WHERE (id1 = %d AND id2 = %d) OR (id1 = %d AND id2 = %d);", id1, id2, id2, id1); // Deleting friend 
    sqlite3_prepare_v2(db, sql, -1, &selectstmt, NULL);                            
    sqlite3_step(selectstmt);
    sqlite3_finalize(selectstmt);
    sqlite3_close(db);
    return 1;
